...
import { 
  ...
  ChevronRight, ChevronLeft, Package, Clock, MoreHorizontal,
  Edit2, Trash2, CheckCircle, Plus
} from 'lucide-react';
...
const ProfileScreen: React.FC<ProfileScreenProps> = ({ 
  ...
  onOrderAgain, onUpdateAddresses, onUpdateCards, onLogout
}) => {
  ...
  const renderHeader = (title: string) => (
    <div className="flex items-center px-4 py-4 pt-10 bg-app-bg sticky top-0 z-20 border-b border-app-divider/50">
      <button 
        onClick={() => setView(ProfileView.Menu)}
        className="p-2 -ml-2 rounded-full hover:bg-black/5 transition-colors"
      >
        <ChevronLeft size={24} className="text-app-dark" />
      </button>
      <h1 className="flex-1 text-center text-lg font-semibold text-app-dark pr-8">
        {title}
      </h1>
    </div>
  );
      ...
      <div className="flex flex-col h-full bg-app-bg animate-in slide-in-from-right-8 duration-300">
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-24 no-scrollbar pt-4">
          <div className="flex flex-col gap-4">
            {orders.map(order => (
              <div key={order.id} className="bg-white rounded-[24px] p-4 shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <span className="text-xs font-semibold text-app-primary bg-app-primary/10 px-2 py-1 rounded-md">
                      {order.id}
                    </span>
                ...
                <div className="border-t border-gray-100 pt-3 flex justify-between items-center">
                   ...
                   <button 
                     ...
                     className="text-sm font-medium text-app-primary hover:text-app-dark transition-colors"
                   >
                     Order again
                   </button>
                </div>
            ...
            {orders.length === 0 && (
              <p className="text-center text-gray-400 mt-10">No past orders found.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col h-full bg-app-bg animate-in slide-in-from-right-8 duration-300">
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-24 no-scrollbar pt-4">
          <div className="flex flex-col gap-3">
               ...
               <div key={addr.id} className={`bg-white rounded-[24px] p-4 shadow-sm border-2 ${addr.isDefault ? 'border-app-primary/50' : 'border-transparent'}`}>
                 <div className="flex justify-between items-start mb-2">
                   <div className="flex items-center gap-2">
                     ...
                     {addr.isDefault && (
                       <span className="text-[10px] bg-app-primary text-app-dark px-1.5 py-0.5 rounded font-medium">Default</span>
                     )}
                   </div>
                   <div className="flex gap-2">
                      <button className="p-1.5 text-gray-400 hover:text-app-dark bg-gray-50 rounded-lg">
                        <Edit2 size={14} />
                      </button>
                      <button 
             ...
             <button className="w-full py-4 border border-dashed border-gray-300 rounded-[24px] text-gray-400 font-medium flex items-center justify-center gap-2 hover:border-app-primary hover:text-app-primary hover:bg-white transition-all">
                <Plus size={18} />
                Add new address
             </button>
          </div>
          ...
          <div className="flex flex-col gap-3">
             {cards.map(card => (
               <div key={card.id} className="bg-white rounded-[24px] p-4 shadow-sm flex items-center gap-4">
                 <div className="w-12 h-8 bg-app-dark/5 rounded flex items-center justify-center">
                    <CreditCard size={18} className="text-app-dark" />
                 </div>
                 ...
                 {!card.isDefault ? (
                   <button 
                     ...
                     className="p-2 text-gray-300 hover:text-app-primary"
                   >
                     <div className="w-4 h-4 rounded-full border border-current" />
                   </button>
                 ) : (
                    <button className="p-2 text-app-primary">
                        <div className="w-4 h-4 rounded-full bg-app-primary border border-app-primary" />
                    </button>
                 )}
               </div>
             ))}
            ...
            <div className="p-6">
                <div className="bg-white rounded-[24px] p-6 shadow-sm text-center">
                    ...
                    <p className="text-sm text-gray-500 mb-6">
                        If you have any issues with your order or the app, please contact our support team.
                    </p>
                    <button className="w-full bg-app-bg text-app-dark font-medium py-3 rounded-[24px] mb-3">
                        Email Support
                    </button>
                    <button className="w-full bg-white border border-gray-200 text-gray-400 font-medium py-3 rounded-[24px] cursor-not-allowed">
  ...
  return (
    <div className="flex flex-col h-full bg-app-bg animate-in fade-in duration-300">
      <div className="px-6 pt-12 pb-8 flex flex-col items-center">
        <div className="w-24 h-24 rounded-full p-1 border-2 border-app-primary mb-4 shadow-lg shadow-app-primary/20">
            <img 
              src={user.avatarUrl} 
              alt={user.name} 
        ...
        <h2 className="text-xl font-bold text-app-dark">{user.name}</h2>
        <p className="text-sm text-gray-500">{user.email}</p>
      </div>

      <div className="flex-1 px-5 pb-24 overflow-y-auto no-scrollbar">
        <div className="bg-white rounded-[32px] shadow-card p-2">
            {[
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between p-4 hover:bg-gray-50 rounded-[24px] transition-colors group"
                >
                    <div className="flex items-center gap-4">
                        ...
                        <span className="font-medium text-app-dark">{item.label}</span>
                    </div>
            ...
            <button 
                onClick={onLogout}
                className="w-full flex items-center justify-between p-4 hover:bg-red-50 rounded-[24px] transition-colors group"
            >
                <div className="flex items-center gap-4">
                    ...
                    <span className="font-medium text-gray-500 group-hover:text-red-500">Log out</span>
                </div>
            </button>
        </div>
        
        <div className="text-center mt-8 text-xs text-gray-300">
            Version 1.0.0 • CookEasy Inc.
        </div>
      </div>
    </div>
  );
